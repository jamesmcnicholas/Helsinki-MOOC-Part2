...
public class FirstPackagesTest {
    ...
    public void hasUserInterface() {
        Class clazz = null;
        try {
            clazz = ReflectionUtils.findClass("mooc.mooc.ui.ui.mooc.UserInterface");
        } catch (Throwable e) {
            fail("Create package mooc.mooc.ui.ui and inside it, create interface mooc.UserInterface:\n\n"
                    + "public interface mooc.UserInterface{\n  \\\\...\n}");
        }

        if (clazz == null) {
            fail("Have you created public interface mooc.UserInterface inside the package mooc.mooc.ui.ui?");
        }

        if (!clazz.isInterface()) {
            fail("Are you sure that mooc.UserInterface is an interface?");
        }
        ...
        if (!loytyi) {
            fail("Does the interface mooc.UserInterface have method void update()?");
        }
    }
    ...
    public void hasTextUserInterface() {
        Class clazz = ReflectionUtils.findClass("mooc.mooc.TextUserInterface");

        if (clazz == null) {
            fail("Have you created public class TextUserInterface inside the package mooc.mooc.ui.ui?");
        }

        Class ui = ReflectionUtils.findClass("mooc.mooc.ui.ui.mooc.UserInterface");
        ...
        fail("Does the class TextUserInterface implement interface mooc.UserInterface?");
    }
    ...
    public void textUserInterfacePrints() {
        ...
        try {
            ...
            fail("There was an error when executing code. Check what happens when you run code:\n"
                    + "mooc.UserInterface ui = new TextUserInterface();\n"
                    + "ui.update()\n");
        }
    ...
    public void hasApplicationLogic() {
        ...
        if (sovellusLogiikkaClass == null) {
            fail("Have you created public class ApplicationLogic inside the package mooc.mooc.logic?");
        }
        Class uiClass = ReflectionUtils.findClass("mooc.mooc.ui.ui.mooc.UserInterface");

        Constructor constructor = ReflectionUtils.requireConstructor(sovellusLogiikkaClass, uiClass);
        if (constructor == null) {
            fail("Does the class ApplicationLogic have constructor public ApplicationLogic(mooc.UserInterface ui)?");
        }
    ...
    public void noRedundantVariables() {
        saniteettitarkastus("mooc.ApplicationLogic", 1, "an instance variable of type mooc.UserInterface");
    }
    ...
    public void createApplicationLogicObject() throws IllegalArgumentException, IllegalAccessException {
        ...
        assertTrue("Does the class ApplicationLogic have an instance variable of type mooc.UserInterface?", kentat.length == 1);
        assertTrue("Does the class ApplicationLogic have an instance variable of type mooc.UserInterface?", kentat[0].toString().contains("mooc.UserInterface"));
        ...
        assertFalse("ApplicationLogic's instance variable " + kentat[0].toString().replace("mooc.ApplicationLogic.", "") + " "
                + "null. \nSet the mooc.UserInterface-object given as a parameter in the constructor as this instance variable's value!", kentat[0].get(sovelluslogic) == null);
        ...
        if (executeOutput.length() < output.length() * 3) {
            fail("Is the method update() of interface mooc.UserInterface called as many times as the parameter howManyTimes tells to in the method execute of class ApplicationLogic?");
    ...
    private boolean sisaltaaKutsun(String kutsu) {
        ...
        try {
            Scanner lukija = new Scanner(new File("src/mooc/mooc.mooc.logic/ApplicationLogic.java"));
            int metodissa = 0;
